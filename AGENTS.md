...
- Search routes are defined in [itmdatacommons/urls.py](itmdatacommons/urls.py), and the configured `sociome` Globus Search index lives in [itmdatacommons/settings/base.py](itmdatacommons/settings/base.py).
- The root route renders the `sociome` search-about page; `<index>/data` handles search requests, while framework and social-auth routes are included beneath the same URL configuration.
...
- `itmdatacommons` is the active Django package: `ROOT_URLCONF`, WSGI/ASGI, settings, and field imports point there. Treat [sociome_portal/settings/local.py](sociome_portal/settings/local.py) as a legacy or alternate settings location and verify all import paths before changing package names.
